...
with engine.connect() as conn:
    try:
        query = (
            """ SELECT RN, TITLE FROM otodom_data_short_flatten ORDER BY rn limit 300"""
        )

        df = pd.read_sql(query, conn)

        gc = gspread.service_account(
            filename=r"C:/Users/almas/OneDrive/Desktop/sample/Python_scripts/service_account.json.json"
        )

        loop_counter = 0
        chunk_size = 100
        file_name = "OTODOM_ANALYSIS_"
        user_email = "elearn250@gmail.com"
